...
"""
This script fine-tunes the SENet50 backbone of VGGFace for memorability prediction. 
It includes dropout layers for regularization and employs a custom Euclidean distance loss. 
The script uses transfer learning and implements callbacks for saving models and adjusting 
...
output = Dense(1)(drop)  # Add a dense layer with 1 neuron for regression (memorability score prediction)
...
callbacks = [
    tf.keras.callbacks.ModelCheckpoint(filepath="senet_hf/drop/epoch_{epoch}.h5"),  # Save model checkpoints
    tf.keras.callbacks.CSVLogger("senet_hf/drop/faces_ft1.log", separator=",", append=False),  # Log training metrics
...
model.fit_generator(
    ...
    validation_steps=int(len(test_split) / batch_size),  # Number of validation steps
