...
It incorporates training and validation pipelines, data generators, and callbacks 
for monitoring and saving model performance during training.
"""
...
model = VGGFace(model='resnet50')  # Use ResNet50 backbone
...
callbacks = [
    tf.keras.callbacks.ModelCheckpoint(filepath="res_hf/final/epoch_{epoch}.h5"),  # Save model checkpoints
    tf.keras.callbacks.CSVLogger("res_hf/final/faces_ft.log", separator=",", append=False),  # Log training metrics
